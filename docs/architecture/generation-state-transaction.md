...
Add and non-empty replace both use this same transaction path. Replace can include safe managed deletions, which are restored with ownership and selection on rollback or pre-commit crash recovery. Remove, clear, prune, historical full-output bootstrap, output migration, and caller-selected output or cleanup policy remain unsupported. Full plans continue to use journal v1 and `commitOutputTransaction()`; only selective plans with the controlled selection state use journal v2.
