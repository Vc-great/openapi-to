...
Configured output may use the default managed base below `.openapi-to` or `base: 'workspace'` below the project root. The shared Core preflight rejects unsafe or overlapping output roots before generation. Ownership follows the resolved output root; persistent Operation selection remains in `.openapi-to/selections`. Prepare does not create either location.
