...
When adding discovered operations to persistent project intent, Codex may pass `selection: { type: "add", operationKeys: [...] }` to Prepare. It should compare the returned previous/requested/new/already-selected/desired summaries and projection counts. This B1 plan has `applySupported: false` and no token: Codex must not attempt Apply. It writes neither selection nor generated output. Omit `selection` for the existing full Prepare/Apply workflow.
