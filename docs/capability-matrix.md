...
| --- | --- | --- | --- |
| TypeScript types | Stable | `@openapi-to/plugin-ts-type` / `pluginTSType` | Component and operation types. Path/query/header/cookie parameters share requiredness and Parameter Object `schema`/first-declared-`content` handling. Response Media Types without `schema` map to `unknown`, responses without content map to `undefined`, and per-status members remain declared before aggregation. Request-body schema `$ref` values pass static siblings such as `nullable`, type unions, `oneOf`/`anyOf`, and `allOf` through the schema renderer. |
