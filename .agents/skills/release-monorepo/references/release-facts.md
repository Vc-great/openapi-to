...
- `.github/workflows/publish.yml` is the maintained manual publication path. It validates the exact `main` SHA, fixed-group version, `rc`/`latest` channel, and dist-tag; runs readiness before registry authority; publishes through the `npm-production` Environment and npm Trusted Publishing/OIDC; verifies every package in the registry; and only then creates the version tag and GitHub Release.
