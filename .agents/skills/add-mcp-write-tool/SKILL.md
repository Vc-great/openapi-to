...
Use the official stable SDK Client and stdio transport against the built binary for Prepare, Apply, disk verification, replay, invalid input, timeout/cancellation, stderr redaction, and stdout purity. Check current Inspector help before smoke. Run real Codex safety cases and require zero Apply calls without explicit confirmation, no plan guessing/replay, and deletion disclosure.

Then run affected Core/MCP/CLI tests/typechecks/builds, formal second-run generation check, root Vitest/typecheck/build, changed-file lint, release scripts, package surface, tarball install/write smoke, and Changeset status. Inspect the MCP tarball for fixtures, journals, failpoints, tokens, benchmarks, logs, local config, or machine paths. Never publish, commit, push, or tag without explicit authorization.
