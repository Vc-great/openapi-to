...
9. Preserve stdio: stdin/stdout are MCP JSON-RPC only. Route logs and plugin incidental console output to stderr without replacing `process.stdout.write` or installing per-call console restore races.
10. Run focused unit/security tests and a real official SDK `Client` + `StdioClientTransport` subprocess test against the built bin. Inspect `tools/list`, schemas, annotations, success/error calls, stderr, shutdown, determinism, and no filesystem mutation.
11. Check the current official MCP Inspector help and run a stdio smoke. Run a current Codex project-config smoke when the task requires it; document observed results, not assumptions.
12. Update README/ADR/AGENTS/Changeset/release scripts when public package behavior or surface changes. Run package test/typecheck/build, root tests/typecheck/build, changed-file lint, package-surface verification, pack-install smoke, and Changesets status as impact requires.
