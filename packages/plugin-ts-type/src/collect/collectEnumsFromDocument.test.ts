...
describe("collectEnumsFromDocument", () => {
	describe("collectEnumsFromSchema", () => {
		...
		it("collects enum and nested enum siblings beside a reference", () => {
			const schema = {
				$ref: "#/components/schemas/BaseStatus",
				enum: ["ACTIVE", "INACTIVE"],
				properties: {
					nested: { type: "string", enum: ["ONE", "TWO"] },
				},
			};
		...
		it("collects an enum sibling beside an array item reference", () => {
			const result = collectEnumsFromSchema(
				{
					type: "array",
					items: {
						$ref: "#/components/schemas/BaseStatus",
		...
		it("应该处理无效输入", () => {
			...
			expect(
				collectEnumsFromSchema(
					"string" as unknown as Parameters<typeof collectEnumsFromSchema>[0],
					"Test",
				),
			).toHaveLength(0);
		});
	...
	describe("collectEnumsFromPathRequestBodies", () => {
		...
		it("应该处理对象请求体", () => {
			...
			const result = collectEnumsFromPathRequestBodies(
				requestBody,
				"createUser",
			);
			expect(result).toHaveLength(1);
		...
		it("应该处理无schema的请求体", () => {
			...
			const result = collectEnumsFromPathRequestBodies(
				requestBody as unknown as Parameters<
					typeof collectEnumsFromPathRequestBodies
				>[0],
				"createUser",
			);
			expect(result).toHaveLength(0);
	...
	describe("collectEnumsFromPathResponses", () => {
		it("应该从响应中收集枚举", () => {
			...
			expect(result).toHaveLength(1);
			expect(result[0].name).toBe("getUsersOK");
	...
	describe("collectEnumsFromComponentRequestBody", () => {
		it("应该从组件请求体中收集枚举", () => {
			...
			const result = collectEnumsFromComponentRequestBody(
				requestBody,
				"Format",
			);
			expect(result).toHaveLength(1);
