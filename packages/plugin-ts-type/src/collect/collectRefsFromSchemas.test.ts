...
describe("collectRefsFromSchema", () => {
	...
	it("应该处理边缘情况 - 非对象值", () => {
		// @ts-expect-error Verifies the defensive runtime boundary for invalid input.
		expect(collectRefsFromSchema("string")).toEqual([]);
		// @ts-expect-error Verifies the defensive runtime boundary for invalid input.
		expect(collectRefsFromSchema(123)).toEqual([]);
		expect(collectRefsFromSchema(true)).toEqual([]);
	});
	...
	it("collects refs from schema siblings beside $ref", () => {
		const schema = {
			$ref: "#/components/schemas/User",
			properties: {
