...
vi.mock("@/utils/formatterRef", () => ({
	getRefFileNameFormComponentRequestBodies: vi
		.fn()
		.mockImplementation((ref) => `MockFormatted_${ref.split("/").pop()}`),
}));
...
vi.mock("@/templates/requestBodyTemplate", () => ({
	requestBodyTemplate: vi
		.fn()
		.mockImplementation((name, _body) => ({ name, bodyType: "mocked" })),
}));
...
describe("buildComponentsRequestBody", () => {
	...
	it("应该处理带有引用schema的请求体", () => {
		...
		expect(requestBodyTemplate).toHaveBeenCalledWith(
			"RequestBodiesTestSchemaModel",
