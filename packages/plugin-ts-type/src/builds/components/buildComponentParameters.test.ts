...
vi.mock("@/templates/schemaTemplate", () => ({
	schemaTemplate: vi
		.fn()
		.mockImplementation((_schema, name) => `MockSchemaType_${name}`),
}));

vi.mock("@/templates/jsDocTemplateFromSchema", () => ({
	jsDocTemplateFromSchema: vi
		.fn()
		.mockImplementation((_desc, _schema, name) => [`Mock JSDoc for ${name}`]),
}));
...
describe("buildComponentParameters", () => {
	...
	it("应该处理带有引用schema的参数", () => {
		...
		expect(schemaTemplate).toHaveBeenCalledWith(
			parameter.schema,
			"ParameterTestModel",
		);
		expect(createTypeAlias).toHaveBeenCalledWith(
			"ParameterTestModel",
			"MockSchemaType_ParameterTestModel",
			["Mock JSDoc for test"],
		);
		expect(result).toEqual({
			name: "ParameterTestModel",
	...
	it("应该处理带有内联schema的参数", () => {
		...
		expect(jsDocTemplateFromSchema).toHaveBeenCalledWith(
	...
	it("uses the compatibility string fallback when schema and content are absent", () => {
		// @ts-expect-error Exercises the documented generation fallback for an invalid Parameter Object.
		const result = buildComponentParameters({}, "test");
		expect(result).toEqual({
			name: "ParameterTestModel",
			type: "MockSchemaType_ParameterTestModel",
			docs: ["Mock JSDoc for test"],
