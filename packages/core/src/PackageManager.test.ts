...
describe("getPackageJSON", () => {
  ...
  test("if version out of package.json data is returned", async () => {
    const version = await packageManager.getVersion("axios");

    expect(version?.startsWith("^1.")).toBeTruthy();

    const versionRegex = await packageManager.getVersion("axi");

    expect(versionRegex?.startsWith("^1.")).toBeTruthy();
  });
  ...
  test("normalizeDirectory", () => {
    expect(packageManager.normalizeDirectory("/user/nzakas/foo")).toBe(
      "/user/nzakas/foo/",
    );
    expect(packageManager.normalizeDirectory("/user/nzakas/foo/")).toBe(
      "/user/nzakas/foo/",
  ...
  test("it should find mocks/noop.js file with default cwd and ESM", async () => {
    ...
    const module = await packageManager.import(
      path.join(__dirname, "../mock/noop.js"),
    );
  ...
  test("it should find mocks/noop.js file with default cwd and CJS", async () => {
    ...
    const module = await packageManager.import(
      path.join(__dirname, "../mock/noop.cjs"),
    );
