...
function prependGeneratedTypeScriptHeader(source: string): string {
  return source.startsWith(GENERATED_TYPESCRIPT_HEADER) ? source : `${GENERATED_TYPESCRIPT_HEADER}${source}`
}
  ...
  for (const artifact of sortGeneratedArtifacts(artifacts)) {
    ...
    try {
      if (artifact.kind === 'typescript') {
        ...
        content = encoder.encode(prependGeneratedTypeScriptHeader(artifact.sourceFile.getFullText()))
