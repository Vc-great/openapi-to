...
export class Parameter {
  ...
  get hasQueryParametersArray(): boolean {
    return _.some(this.parameters, (parameter) => 'schema' in parameter && parameter.schema && 'type' in parameter.schema && parameter.schema.type === 'array')
  }

  get url(): string {
    const path = this.operation.path.replace(/{([\w-]+)}/g, (_matchData, pathParams: string) => {
