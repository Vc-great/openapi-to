...
export class OpenAPI {
  ...
  setCurrentOperation(path: string, method: HttpMethod, tagName: string): Operation {
    this.operation = this.oas.operation(path, method)
    //
    this.currentTagMetadata = this.operation.getTags().find((tag) => tag.name === tagName)
    //
    ...
    this.schemas = new Schema(this.operation)
  ...
  private generateRequestName(): string {
    ...
    return _.camelCase(paths[paths.length - 1]) + _.upperFirst(this.operation.method)
  }
