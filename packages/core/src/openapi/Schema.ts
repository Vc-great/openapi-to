...
export class Schema {
  ...
  //todo 优化
  getComponent(ref: string = ''): OpenAPIV3.SchemaObject | OpenAPIV3.ReferenceObject {
    return _.get(this.openAPI.openApi3SourceData, ref.split('/').slice(1).join('.'))
  }

  getComponentResolveRefCache(ref: string = ''): OpenAPIV3.SchemaObject | OpenAPIV3.ReferenceObject | undefined {
  ...
  handleComponentSchema(
    ...
    if ('$ref' in schemaObject && schemaObjectHas$Ref) {
      ...
      const component = this.getComponent(schemaObject.$ref) as OpenAPIV3.SchemaObject

      return schemaObjectHas$Ref({ schemaObject, component, parent, key })
    }

    // 数组类型
    if ('type' in schemaObject && schemaObject.type === 'array' && '$ref' in schemaObject.items) {
      this.pendingRefCache.add(schemaObject.items.$ref)

      const componentBySchemaObjectItemsRef = this.getComponent(schemaObject.items.$ref)
      if (componentBySchemaObjectItemsRef && '$ref' in componentBySchemaObjectItemsRef && arraySchemaObjectItemsHas$RefOfComponent) {
        this.pendingRefCache.add(componentBySchemaObjectItemsRef.$ref)

        const component = this.getComponent(componentBySchemaObjectItemsRef.$ref) as OpenAPIV3.SchemaObject
      ...
      if (arraySchemaObjectItemsHas$Ref && !('$ref' in componentBySchemaObjectItemsRef)) {
    ...
    //array 没有ref
    if ('type' in schemaObject && schemaObject.type === 'array' && !('$ref' in schemaObject.items) && arrayItemsNo$ref) {
    ...
    // 对象类型 properties 不存在
    if ('type' in schemaObject && schemaObject.type === 'object' && !schemaObject.properties) {
    ...
    // 对象类型
    if ('type' in schemaObject && schemaObject.type === 'object' && schemaObject.properties && objectHasProperties) {
    ...
    // 继承类型
    if ('allOf' in schemaObject && schemaObject.allOf && schemaObject.allOf.length) {
      //todo 待补充
    ...
    //基本类型
    if ('type' in schemaObject && ['integer', 'number'].includes(schemaObject.type || '')) {
      return baseOfNumber({
        schemaObject,
        parent,
        key,
