...
Prepare also accepts `selection: { type: 'add', operationKeys: [...] }`. It reads the internally derived `.openapi-to/selections/<owner>.json`, fails closed on ambiguous bootstrap/history drift, unions previous and requested keys, and generates the complete desired projection. Selection semantic state, its physical before-snapshot, exact desired serialized bytes, projection, artifacts, ownership bytes, target, output identity, and plan kind enter the plan/token binding. A successful selective Prepare returns `applySupported: true` and a one-time token while writing nothing. After explicit review, Apply compiles the trusted target afresh, regenerates exactly the frozen complete desired key set, revalidates selection twice (including once under the output lock), compares projection/artifact/ownership identities, and calls Core's three-state transaction for generated artifacts, ownership, and selection. Only `add` is supported; full Prepare/Apply remains unchanged. See [persistent operation selection](../../docs/architecture/persistent-operation-selection.md) and [generation state transaction](../../docs/architecture/generation-state-transaction.md).
