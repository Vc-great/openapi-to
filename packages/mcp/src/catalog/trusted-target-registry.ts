...
function schemaCount(compilation: OpenAPICompilation): number {
	const root = compilation.normalizedDocument as
		| Record<string, unknown>
		| undefined;
	const components =
		typeof root?.components === "object" && root.components !== null
...
export class TrustedTargetCatalogRegistry {
	...
	async targetNames(signal?: AbortSignal): Promise<string[]> {
		...
		return prepared.targets.map(({ name }) => name);
	}
	...
	async resolveTargetName(
		...
		if (requested) {
			if (!names.includes(requested))
				throw new McpToolError(
					"MCP_UNKNOWN_TARGET",
					`Unknown configured target: ${requested}`,
				);
			return requested;
		}
		...
		if (names.length === 0)
			throw new McpToolError(
				"MCP_CONFIG_NO_TARGETS",
				"The trusted configuration has no OpenAPI targets.",
			);
		throw new McpToolError(
			"MCP_TARGET_REQUIRED",
			"A target is required when the trusted configuration contains multiple targets.",
			"Call openapi_list_targets, then select one returned target name.",
		);
	}
	...
	async get(
		...
		if (!selected)
			throw new McpToolError(
				"MCP_UNKNOWN_TARGET",
				`Unknown configured target: ${target}`,
			);
		...
		void pending.then(
			(result) => {
				if (!result.success && this.cache.get(key) === pending)
	...
	private async compileTarget(
		...
		const rootSnapshot = compilation.references?.sourceSnapshots.find(
			(snapshot) => snapshot.isRoot,
		);
		const catalog = compilation.document
			? buildOperationCatalog(compilation.document, {
					target: target.name,
					resolvedDocument:
						compilation.normalizedDocument ?? compilation.resolvedDocument,
