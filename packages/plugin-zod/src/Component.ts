...
import { ZOD_MODEL_FOLDER_NAME, ZOD_SUFFIX } from './constants.ts'
import { formatFileName, formatRefName, formatZodName, formatZodTypeName } from './utils.ts'
...
export class Component {
  ...
  generateCodeByComponentRef(schema: OpenAPIV3.ReferenceObject, key: string): void {
    //    const name = key + _.upperFirst(ZOD_SUFFIX);
    const name = formatZodName(key)
    const refName = formatRefName(this.openapi.getRefAlias(schema.$ref))
    ...
    this.createModelSourceFile(statements, variableDeclaration?.getName() ?? name)
    //
    this.setModelIndexStatements(statements)
  }

  createModelSourceFile(statements: Array<StatementStructures>, fileName: string): void {
    const filePath = path.resolve(this.openapiToSingleConfig.output.dir || './', `${this.modelFolderName}/${fileName}.ts`)
  ...
  setModelIndexStatements(statements: Array<StatementStructures>): void {
    ...
    const names = _.chain(statements)
      ...
      .map((item: VariableStatementStructure) => _.get(item, 'declarations[0].name', ''))
  ...
  generateModelIndex(): void {
    const statements = _.chain([...this.modelIndex])
      .map((name) => {
        const fileName = name.endsWith(_.upperFirst(ZOD_SUFFIX)) ? name.replace(_.upperFirst(ZOD_SUFFIX), '') : name

        return this.ast.generateExportStatements({
          moduleSpecifier: `./${formatFileName(fileName)}`,
        })
      })
      .value()
  ...
  generateCodeByComponentSchema(schema: SchemaObject, key: string): void {
    ...
    const zodName = formatZodName(key)
    ...
    const importStatements = _.chain([...this.openapi.refCache.keys()])
      .map((ref) => {
        const UUID = UUIDPrefix + formatRefName(this.openapi.getRefAlias(ref))
        const declaration = this.oldNode.declarationCache.get(UUID)
        const name = declaration?.getName() ?? formatRefName(this.openapi.getRefAlias(ref))

        return this.ast.generateImportStatements([
          ...(name === formatZodName(key)
            ? []
            : [
                {
                  namedImports: [name],
                  isTypeOnly: false,
                  moduleSpecifier: `./${formatFileName(this.openapi.getRefAlias(ref))}`,
                },
              ]),
        ])
      })
    ...
    const zodTypeStatements = this.ast.generateTypeAliasStatements({
      name: formatZodTypeName(key),
      type: this.z.head().infer(zodName).toString(),
      docs: docs,
      isExported: true,
    })

    const statements = [...importStatements, ...this.enumGenerator.generateEnum(), ...zodImport, schemaStatements, zodTypeStatements]

    this.createModelSourceFile([...statements], formatFileName(key))
    this.setModelIndexStatements([...importStatements, ...zodImport, schemaStatements, zodTypeStatements])
  }
