...
export async function generate(
	...
	const logger = CLIOptions.json
		? undefined
		: createLogger({
				logLevel: CLIOptions.logLevel ?? LogMapper.info,
				name: openapiToSingleConfig.name,
			});
	if (!CLIOptions.json)
		logger?.emit("start", `Building ${openapiToSingleConfig.input.path}`);
	const result = compilation
		? await buildFromCompilation(
				openapiToSingleConfig,
				compilation,
				CLIOptions,
				logger,
			)
		: await build(openapiToSingleConfig, CLIOptions, logger);
	if (!CLIOptions.json) {
		...
		if (result.error)
			logger?.consola?.error(
				`Build failed (${summary.errors} errors, ${summary.warnings} warnings)`,
			);
		else
			logger?.consola?.success(
				`Build completed (${manifest?.added ?? 0} added, ${manifest?.modified ?? 0} modified, ${manifest?.deleted ?? 0} deleted, ${manifest?.unchanged ?? 0} unchanged)`,
			);
	}
	return {
		name: openapiToSingleConfig.name,
